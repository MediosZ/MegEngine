/**
 * \file dnn/src/wasm/batched_matrix_mul/algos.cpp
...
#include "src/wasm/batched_matrix_mul/algos.h"
...
void BatchedMatrixMulForwardImpl::AlgoDefault::exec(
    ...
    auto kern = [args, param]() {
        auto A = args.tensor_a;
        auto B = args.tensor_b;
        auto C = args.tensor_c;
        // use xnnpack
        if(!param.transposeA && !param.transposeB && A.layout.shape[0] == 1 && B.layout.shape[0] == 1){
            float output_min = -std::numeric_limits<float>::infinity();
            float output_max = std::numeric_limits<float>::infinity();
            const size_t input_channels = B.layout.shape[1];
            const size_t output_channels = B.layout.shape[2];
            ...
            const size_t batch_size = A.layout.shape[1];
