/**
 * \file dnn/src/wasm/conv_bias/im2col/algos.cpp
...
#include "src/wasm/conv_bias/im2col/im2col_kerns.h"
#include "src/wasm/conv_bias/im2col/factory.h"
...
MIDOUT_DECL(megdnn_wasm_im2col)
...
namespace {
static wasm::MatrixMulImpl::KernSizeParam get_matmul_kern_param(
        const wasm::ConvBiasImpl::NCBKernSizeParam& param,
...
static void choice_ohw_oc_block(
        ...
        wasm::MatrixMulImpl::AlgoBase::PackMode pack_mode) {
    ...
    if (nr_threads > 1) {
        ...
        if (pack_mode == wasm::MatrixMulImpl::AlgoBase::PackMode::NO_PACK) {
            ohw_tile_size = ohw;
            oc_tile_size = OC;
        }
    }
...
static size_t packA_group_size(
        ...
        const wasm::MatrixMulImpl::KernSizeParam& matmul_param,
        const wasm::MatrixMulImpl::AlgoBase::MatmulDescription& matmul_desc,
        size_t packa_parallel_times) {
    if (matmul_desc.packmode ==
        wasm::MatrixMulImpl::AlgoBase::PackMode::DEFAULT) {
        return matmul_algo->get_bundle(matmul_param).get_size(0);
    } else if (matmul_desc.packmode ==
               wasm::MatrixMulImpl::AlgoBase::PackMode::ONLY_PACKA) {
        return packa_parallel_times *
               matmul_algo->get_bundle(matmul_param).get_size(0);
    }
    megdnn_assert(matmul_desc.packmode ==
                  wasm::MatrixMulImpl::AlgoBase::PackMode::NO_PACK);
...
static WorkspaceBundle get_bundle(
    ...
    wasm::MatrixMulImpl::AlgoBase::MatmulDescription matmul_desc =
...
size_t ConvBiasImpl::AlgoIm2col::get_workspace(
    ...
    MIDOUT_BEGIN(megdnn_wasm_im2col, 0, 0) {
        wasm::MatrixMulImpl::AlgoBase::MatmulDescription matmul_desc =
...
SmallVector<ConvBiasImpl::NCBKern> ConvBiasImpl::AlgoIm2col::dispatch_kerns(
    ...
    MIDOUT_BEGIN(megdnn_wasm_im2col, 0, 1) {
        ...
        if (default_pack) {
            MIDOUT_BEGIN(megdnn_wasm_im2col,
...
bool ConvBiasImpl::AlgoIm2col::usable(
    ...
    MIDOUT_BEGIN(megdnn_wasm_im2col, 0, 2) {
        ...
        choice_ohw_oc_block(param, oc_tile_size, ohw_tile_size,
                            ...
                            matmul_desc.innerblocksize.n, m_ohw_tile_size,
                            matmul_desc.packmode);
        wasm::MatrixMulImpl::KernSizeParam matmul_param =
...
ConvBiasImpl::AlgoIm2col::dispatch_preprocess_kerns(
        const NCBKernSizeParam& param) const {
    MIDOUT_BEGIN(megdnn_wasm_im2col, 0, 3) {
        size_t OC = param.filter_meta.ocpg;
        size_t oc_tile_size = 0, ohw_tile_size = 0;
        size_t GROUP = param.filter_meta.group;
        wasm::MatrixMulImpl::AlgoBase::MatmulDescription matmul_desc =
