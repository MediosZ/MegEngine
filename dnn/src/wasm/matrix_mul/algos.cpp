/**
 * \file dnn/src/wasm/matrix_mul/algos.cpp
...
#include "src/wasm/matrix_mul/algos.h"
#include "megdnn/opr_param_defs.h"
#include "src/wasm/matrix_mul/gemm_impl.h"
#include "src/wasm/matrix_mul/gemv.h"
#include "src/wasm/matrix_mul/generic_strategy.h"

#include "src/naive/matrix_mul/matrix_mul_helper.h"

#include "midout.h"

MIDOUT_DECL(megdnn_wasm_matmul_f32_kern)
MIDOUT_DECL(megdnn_wasm_matmul_f32_gemm_gemv_like)
MIDOUT_DECL(megdnn_wasm_matmul_naive)
...
void f32_8x12x1_kern(const MatrixMulImpl::KernParam& kern_param) {
    MIDOUT_BEGIN(megdnn_wasm_matmul_f32_kern, void) {
        ...
        matmul::wasm::sgemm_8x12 strategy(M, N, K, kern_param.A_type,
                                              kern_param.B_type,
                                              kern_param.C_type);
        matmul::GemmInterleaved<matmul::wasm::sgemm_8x12>(
...
size_t MatrixMulImpl::AlgoF32K8x12x1::get_workspace(
        const KernSizeParam& kern_size_param) const {
    MIDOUT_BEGIN(megdnn_wasm_matmul_f32_kern,
                 midout_iv("AlgoF32K8x12x1::get_workspace"_hash)) {
        auto M = kern_size_param.M, N = kern_size_param.N,
             K = kern_size_param.K;
        matmul::wasm::sgemm_8x12 strategy(M, N, K, kern_size_param.A_type,
                                              kern_size_param.B_type,
                                              kern_size_param.C_type);
        return matmul::GemmInterleaved<matmul::wasm::sgemm_8x12>(
...
MEGDNN_REG_GEMM_FUNC_FOR_IM2COL_IMPL(AlgoF32K8x12x1, megdnn_wasm_matmul_f32_kern,
                                     5, matmul::wasm::sgemm_8x12, float,
...
#define DISPATCH(A, C, func, _midout_iv)                               \
    if (kern_size_param.A_type.enumv() == DTypeEnum::A &&              \
        ...
        kern_size_param.format == param::MatrixMul::Format::DEFAULT) { \
        MIDOUT_BEGIN(megdnn_wasm_matmul_f32_gemm_gemv_like,              \
    ...
    if (can_be_treated_as_int8x8x32(kern_size_param)) {
        MIDOUT_BEGIN(megdnn_wasm_matmul_f32_gemm_gemv_like, midout_iv(4)) {
