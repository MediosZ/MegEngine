/**
 * \file dnn/test/wasm/convolution.cpp
...
TEST_F(WASM, BENCHMARK_CONVOLUTION_MATRIX_MUL) {
...
TEST_F(WASM, BENCHMARK_CONVOLUTION_MATRIX_MUL_8832) {
    using Param = Convolution::Param;
    auto run = [&](const TensorShapeArray& shapes, Param param) {
        Benchmarker<Convolution> benchmarker_float(handle());
        size_t RUN = 50;
...
TEST_F(WASM, BENCHMARK_CONVOLUTION_MATRIX_MUL_8816) {
    using Param = Convolution::Param;
    auto run = [&](const TensorShapeArray& shapes, Param param) {
        Benchmarker<Convolution> benchmarker_float(handle());
        size_t RUN = 50;
...
TEST_F(WASM, BENCHMARK_CONVOLUTION_BACKWARD_DATA) {
    using Param = ConvolutionBackwardData::Param;
    auto run = [&](const TensorLayoutArray& tensors, Param param) {
        Benchmarker<ConvolutionBackwardData> benchmarker_wasm(handle());
        size_t RUN = 500;
        benchmarker_wasm.set_display(false)
                .set_dtype(0, dtype::Float32{})
                .set_dtype(1, dtype::Float32{})
                .set_times(RUN)
                .set_param(param);
        auto tmatmul = benchmarker_wasm
                               .set_before_exec_callback(
                                       AlgoChecker<ConvolutionBackwardData>(
                                               "DeconvMatmul"))
                               .exec(tensors);
        auto tdirect = benchmarker_wasm
                               .set_before_exec_callback(
                                       AlgoChecker<ConvolutionBackwardData>(
                                               "DeconvDirect"))
                               .exec(tensors);
...
TEST_F(WASM, CONVOLUTION_MATRIX_MUL) {
...
TEST_F(WASM, CONVOLUTION_NAIVE_ALGO_FP16) {
    Checker<Convolution> checker(handle());
    using Param = Convolution::Param;
...
TEST_F(WASM_MULTI_THREADS, CONVOLUTION_NAIVE_WASM) {
    Checker<Convolution> checker(handle());
    using Param = Convolution::Param;
    checker.set_before_exec_callback(
            AlgoChecker<ConvolutionForward>("WASM_ALGO"));
    Param param;
...
TEST_F(WASM_MULTI_THREADS, CONVOLUTION_NAIVE_ALGO) {
    Checker<Convolution> checker(handle());
    using Param = Convolution::Param;
    checker.set_before_exec_callback(
            AlgoChecker<ConvolutionForward>("NAIVE_ALGO"));
...
TEST_F(WASM, CONVOLUTION_MATRIX_MUL_SINT8) {
    Checker<Convolution> checker(handle());
    using Param = Convolution::Param;
...
TEST_F(WASM, CONVOLUTION_BACKWARD_DATA) {
...
TEST_F(WASM, CONVOLUTION_BACKWARD_DATA_INT8_INT8_INT32) {
    Checker<ConvolutionBackwardData> checker(handle());
    using Param = ConvolutionBackwardData::Param;
    Param param;
...
TEST_F(WASM, CONVOLUTION_BACKWARD_DATA_SINT8) {
    Checker<ConvolutionBackwardData> checker(handle());
    using Param = ConvolutionBackwardData::Param;
    Param param;
...
TEST_F(WASM, CONVOLUTION_BACKWARD_DATA_QUINT8) {
    Checker<ConvolutionBackwardData> checker(handle());
    using Param = ConvolutionBackwardData::Param;
    Param param;
...
TEST_F(WASM, CONVOLUTION_BACKWARD_DATA_NAIVE_ALGO) {
    Checker<ConvolutionBackwardData> checker(handle());
    checker.set_before_exec_callback(
